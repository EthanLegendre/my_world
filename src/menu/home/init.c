...
void init_menu_home(my_world_t *world_data)
{
    ...
    load_background(world_data->menu, world_data->main_page_id,
        world_data->tex_menu, world_data->ctx);
    main_menu_init_buttons(world_data, world_data->tex_menu,
        world_data->font_menu, world_data->ctx->window);
}
